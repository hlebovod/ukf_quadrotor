...
%% Integrated velocities to get position
...
Measurement_tran = Measurement_tran + [1 0 1 0 1 0].*(0.01.*randn(length(Measurement_tran),1)); % noisy position measures of 3 cm
...
Q = (1e-5)*eye(12);  % Process noice covariance
R = [0 0.05/3 0 0.1/3 0 0.05/3 0.01 0 0.01 0 0.01 0].*eye(12);        % Measurement noice covariance
...
for a = 2 : sample_tran/sample_rot*size(Measurement_tran,1)
    ...
    if mod(a,freq_rot/freq_tran) == 0
        Measurement_tran_cur = Measurement_tran(floor(a*freq_tran/freq_rot),:); % takes a position and velocity measures each a/4 round number sample as a corresponds to quaternions frequency
    else
        Measurement_tran_cur = Measurement_tran(floor(a*freq_tran/freq_rot+1),:); % keeps the previous measurement value while a/4 is non-integer number
    end
       
    %% Control inputs recorded at 50 Hz
    
    if mod(a,freq_rot/freq_ci) == 0
        u_cur = u(floor(a*freq_ci/freq_rot),:); % same principle as for position and velocity but for a/8
    else
        u_cur = u(floor(a*freq_ci/freq_rot+1),:);
    end
