...
xhat_plus_ekf = x_vector;                                          % initial state estimation (UKF)
p_plus_ekf = diag(x_vector.^2+0.1);                                % initial state uncertainty (UKF)
...
for a = 1 : size(Measurement_tran,1)
    ...
    [x_hat, P, K_ekf] = EKF_quadrotor(x_hat,u(a,:),dt,y,P,Q,R);
    ...
    trace_p_plus_ekfArray = [trace_p_plus_ekfArray trace(p_plus_ekf)];
...
figure(1)
subplot(3,1,1)
plot(t_disp,yArray(1,:),'b',t_disp,xhat_plus_ekfArray(1,:),'--r')
title('Orientations')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\phi Roll(rad)');grid on
subplot(3,1,2)
plot(t_disp,yArray(3,:),'b',t_disp,xhat_plus_ekfArray(3,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\theta Pitch (rad)');grid on
subplot(3,1,3)
plot(t_disp,yArray(5,:),t_disp,xhat_plus_ekfArray(5,:),'--r')
legend('Real','Estimate')
...
figure(2)
subplot(3,1,1)
plot(t_disp,yArray(2,:),t_disp,xhat_plus_ekfArray(2,:),'--r')
title('Angular Velocities')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\phi dot (rad/s)');grid on
subplot(3,1,2)
plot(t_disp,yArray(4,:),t_disp,xhat_plus_ekfArray(4,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\theta dot (rad/s)');grid on
subplot(3,1,3)
plot(t_disp,yArray(6,:),t_disp,xhat_plus_ekfArray(6,:),'--r')
legend('Real','Estimate')
...
figure(3)
subplot(3,1,1)
plot(t_disp,yArray(9,:),t_disp,xhat_plus_ekfArray(9,:),'--r')
title('Positions')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('X (m)');grid on
subplot(3,1,2)
plot(t_disp,yArray(11,:),t_disp,xhat_plus_ekfArray(11,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('Y (m)');grid on
subplot(3,1,3)
plot(t_disp,yArray(7,:),t_disp,xhat_plus_ekfArray(7,:),'--r')
legend('Real','Estimate')
