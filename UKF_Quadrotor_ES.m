...
xhat_plus_ukf = x;                                          % initial state estimation (UKF)
p_plus_ukf = diag(x.^2+0.1);                                % initial state uncertainty (UKF)
...
for k = 1 : a-1
    ...
    [x_hat, P, K_ukf] = UKF_quadrotor(x_hat,u,dt,y,P,Q,R);
...
figure(2)
subplot(3,1,1)
plot(t,xArray(1,:),'b',t,xhat_plus_ukfArray(1,:),'--r')
title('Orientations')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\phi Roll(rad)');grid on
subplot(3,1,2)
plot(t,xArray(3,:),'b',t,xhat_plus_ukfArray(3,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\theta Pitch (rad)');grid on
subplot(3,1,3)
plot(t,xArray(5,:),t,xhat_plus_ukfArray(5,:),'--r')
legend('Real','Estimate')
...
figure(3)
subplot(3,1,1)
plot(t,xArray(2,:),t,xhat_plus_ukfArray(2,:),'--r')
title('Angular Velocities')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\phi dot (rad/s)');grid on
subplot(3,1,2)
plot(t,xArray(4,:),t,xhat_plus_ukfArray(4,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('\theta dot (rad/s)');grid on
subplot(3,1,3)
plot(t,xArray(6,:),t,xhat_plus_ukfArray(6,:),'--r')
legend('Real','Estimate')
...
figure(4)
subplot(3,1,1)
plot(t,xArray(9,:),t,xhat_plus_ukfArray(9,:),'--r')
title('Positions')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('X (m)');grid on
subplot(3,1,2)
plot(t,xArray(11,:),t,xhat_plus_ukfArray(11,:),'--r')
legend('Real','Estimate')
xlabel('Time (sec)');ylabel('Y (m)');grid on
subplot(3,1,3)
plot(t,xArray(7,:),t,xhat_plus_ukfArray(7,:),'--r')
legend('Real','Estimate')
